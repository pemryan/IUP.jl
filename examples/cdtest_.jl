...
w = Base.unsafe_convert(Ptr{Int32}, [Int32(0)])
h = Base.unsafe_convert(Ptr{Int32}, [Int32(0)])
...
@show(w,h)
    ...
    #ctgc.following = 0
    ctgc.foreground = IUP.CD_BLACK
    ctgc.background = IUP.CD_WHITE
#    ctgc.head = C_NULL;
    ...
    w = Base.unsafe_convert(Ptr{Cint}, [ctgc.w])
    h = Base.unsafe_convert(Ptr{Cint}, [ctgc.h])
    cdGetCanvasSize(w, h, Base.unsafe_convert(Ptr{Cdouble}, C_NULL), Base.unsafe_convert(Ptr{Cdouble}, C_NULL))
    ctgc.w = unsafe_load(w)
    ctgc.h = unsafe_load(h)
    ctgc.bpp = cdGetColorPlanes()

    mm = Base.unsafe_convert(Ptr{Cdouble}, [0.0])
    cdPixel2MM(1, 0, mm, Base.unsafe_convert(Ptr{Cdouble}, C_NULL))
    mm = unsafe_load(mm)
    xres = 1.0/mm;
    ctgc.res = xres;
    ...
    #ctgc.clip_mode = IUP.CD_CLIPOFF


    x1 = Base.unsafe_convert(Ptr{Cint}, [ctgc.clip_xmin]);    x2 = Base.unsafe_convert(Ptr{Cint}, [ctgc.clip_xmax])
    y1 = Base.unsafe_convert(Ptr{Cint}, [ctgc.clip_ymin]);    y2 = Base.unsafe_convert(Ptr{Cint}, [ctgc.clip_ymax])
...
function fButtonCB(self::Ptr{Ihandle}, b::Char, e::Integer, x::Integer, y::Integer, r::Integer)
    ...
    cdActivate(ctgc.iup_canvas)
    cdWriteMode(IUP.CD_NOT_XOR)
    cdForeground(convert(Clong, IUP.CD_BLACK))
    cdLineStyle(IUP.CD_CONTINUOUS)
    cdLineWidth(1)
    cdClip(IUP.CD_CLIPOFF)

    yp = Base.unsafe_convert(Ptr{Cint}, [y]);
    cdUpdateYAxis(yp)
